...
public class Download extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	  ...
	  String downloadFilename = filename.substring(filename.indexOf("_") + 1); // 시작하는 위치만 주면 끝까지 출력함 (substring)
	  ...
	  // 다운로드용 응답 헤더 설정
	  response.setHeader("Content-Type","applicaion/octet-stream");
    ...
    while((readByte = in.read(b))!= -1) { 
      out.write(b, 0, readByte);// 0부터 readByte까지 내보내기
    }
    out.close();
    in.close();
